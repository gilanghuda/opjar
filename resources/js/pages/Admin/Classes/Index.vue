...
async function submit() {
    errors.value = {};
    const res = await fetch('/api/classes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form.value) });
    if (res.ok) {
    ...
    <AppLayout>
        <div class="p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <Button @click="showForm = !showForm">{{ showForm ? 'Tutup' : 'Tambah Kelas' }}</Button>
            </div>
            ...
            <Card class="mb-4">
                <template #default>
                    ...
                    <div v-else>
                        <p class="text-sm text-muted">Klik tombol untuk menambah kelas baru.</p>
                    </div>
                </template>
            </Card>
